...
// The tmpdir repo's real HEAD sha — the independent oracle for the
// sha-present render path (never hardcoded; read from the same repo the
// boundary events were rebased onto).
function currentHeadSha(repo: TmpRepo): string {
  return execFileSync("git", ["rev-parse", "HEAD"], { cwd: repo.root, env: gitEnv(repo.home) })
    .toString("utf8")
    .trim();
}
...
describe("ISS-0008 show: the flat timeline, and the three-state outcome", () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(
        sessionRow.sha_before,
        `test setup invariant: expected sha_before to be captured (rebased onto this repo's real HEAD, ${expectedSha})`,
      ).toBe(expectedSha);
      expect(
        output,
        `show's header did not print sha_before (${expectedSha}, the tmpdir repo's real HEAD)`,
      ).toContain(expectedSha);
      if (sessionRow.sha_after !== null) {
        expect(sessionRow.sha_after, "test setup invariant: expected sha_after to match the same rebased HEAD").toBe(
          expectedSha,
        );
        expect(
          output,
          `show's header did not print sha_after (${expectedSha}, the tmpdir repo's real HEAD)`,
        ).toContain(expectedSha);
      }
  ...
  it(
    ...
    async () => {
      ...
      // sha before/after from boundary lines: this is a PLAIN replay, so
      // the recorded cwd is the recording machine's (dead) path — the
      // hook's `git rev-parse` against it legitimately fails and
      // sha_before/sha_after stay ABSENT (docs/issues/ISS-0008.md,
      // "sha-absent vs sha-present"; the sha-PRESENT path is exercised
      // separately in the R11 test above via a rebased live cwd). The
      // independent oracle for the NULL is the ledger row itself
      // (already-shipped, independently-tested ingest machinery — never
      // re-derived here).
      const sessionRow = ledger.sessions.find((s) => s.session_id === sessionId);
      if (!sessionRow) throw new Error("test setup invariant: no session row found after ingest");
      expect(
        sessionRow.sha_before,
        "test setup invariant: expected sha_before to stay ABSENT on a plain replay (dead recorded cwd)",
      ).toBeNull();
      expect(
        output,
        `show's header did not render the ABSENT marker (${ABSENT_MARKER}) for sha_before on a plain replay`,
